  ...
  async handler(chatUpdate) {
    ...
    try {
      ...
      let groupMetadata = m.isGroup
        ? this.chats.get(m.chat).metadata || (await this.groupMetadata(m.chat))
        : {} || {};
      let participants = m.isGroup ? groupMetadata.participants : [] || [];
      let user = m.isGroup ? participants.find((u) => u.jid == m.sender) : {}; // User Data
      let bot = m.isGroup
        ? participants.find((u) => u.jid == this.user.jid)
        : {}; // Your Data
      let isAdmin = user.isAdmin || user.isSuperAdmin || false; // Is User Admin?
      let isBotAdmin = bot.isAdmin || bot.isSuperAdmin || false; // Are you Admin?
      let isBlocked = this.blocklist
        .map((v) => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net")
        .filter((v) => v != this.user.jid)
        .includes(m.sender); // Is User Blocked?
      for (let name in global.plugins) {
        let plugin = global.plugins[name];
        if (!plugin) continue;
        if (plugin.disabled) continue;
        ...
        if ((usedPrefix = (match[0] || "")[0])) {
          ...
          if (
            ...
            this.send2Button(
              m.chat,
              `Yahhh...\nLimit kamu habis, silahkan beli dengan ketik *${usedPrefix}buy* atau klik tombol di bawah.`,
              "4ndrexyz",
              "Beli",
              `${usedPrefix}buy`,
              "Beli Semua",
  ...
  async participantsUpdate({ jid, participants, action }) {
    ...
    switch (action) {
              ...
              text = (
                action === "add"
                  ? (
                      ...
                      .replace("@subject", this.getName(jid))
                      .replace("@desc", groupMetadata.desc)
                  : chat.sBye || this.bye || conn.bye || "Sayonara, @user!\nsemoga tenang dialam sana"
      ...
      case "demote":
        ...
        let banp = fs.readFileSync('./images/logo.png')
...
◇ Tipe: *${Object.keys(m.message.message)[0]}*
◇ Nomor WhatsApp: 
    *${PhoneNumber('+' + who.replace('@s.whatsapp.net', '')).getNumber('international')}*

`.trim(), '4ndrexyz Bot', 'Disable Delete', '.disable delete', {
      quoted: m.message,
      contextInfo: {
    ...
    this.copyNForward(m.key.remoteJid, m.message).catch((e) =>
      console.log(e, m)
    );
    }
  },
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: "Perintah ini hanya dapat digunakan oleh *Owner 4ndrexyz Bot*",
    owner: "Perintah ini hanya dapat digunakan oleh *Owner 4ndreyxz Bot*",
