let handler = async (m, { conn, args, command }) => {
    ...
    if (command.endsWith('all') || command.endsWith('semua')) {
        for (let i = 0; i < chat.length; i++) { // For loops
            await m.reply('Sayonara👋, 4ndrexyz Bot akan keluar dari group', chat[i].jid)
        ...
        await m.reply("Sayonara👋, 4ndrexyz Bot akan keluar dari group", args[0]);
        await conn.groupLeave(args[0])
        await m.reply('Succes!!')
    } else {
        if (!m.isGroup) return global.dfail('group', m, conn)
        await m.reply("Sayonara👋, 4ndrexyz Bot akan keluar dari group", m.chat); // WKWKW pesannya sama semua. gk kreatif :v
