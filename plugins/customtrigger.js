...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  if (text.length > 8) return conn.reply(m.chat, '_Teks Terlalu Panjang! Maksimal 8 huruf !_', m)
  let q = m.quoted ? m.quoted : m
  let mime = (q.msg || q).mimetype || ''
  if (!mime) throw `Reply gambar dengan caption *${usedPrefix + command} ${text}*`
 try {
  ...
  conn.sendMessage(m.chat, stiker, MessageType.sticker, {
   ...
   m.reply('Error, reply gambarnya ya..')
   throw false
  }
}
