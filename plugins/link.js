let handler = async (m, { conn, args }) => {
  ...
  if (!groupMetadata) throw 'groupMetadata is undefined T_T'
  if (!'participants' in groupMetadata) throw 'participants is not defined T_T'
  let me = groupMetadata.participants.find(user => user.jid === conn.user.jid)
  if (!me) throw 'Aku tidak ada di grup itu T_T'
