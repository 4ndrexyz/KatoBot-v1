...
let handler = async (m, { conn, text, participants, usedPrefix, command }) => {
  ...
  for (let user of response.participants.filter(user => Object.values(user)[0].code == 403)) {
    ...
    let teks = `Mengundang @${jid.split('@')[0]} melalui invite..`
    m.reply(teks, null, {
      contextInfo: {
        mentionedJid: conn.parseMention(teks)
      }
    })
    await conn.sendGroupV4Invite(m.chat, jid, invite_code, invite_code_exp, false, 'Mengundang untuk masuk di Grup WhatsApp', jpegThumbnail ? {
      jpegThumbnail
    } : {})
  }
}
